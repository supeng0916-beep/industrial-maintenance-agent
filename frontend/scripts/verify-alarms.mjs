...
const output = resolve(root, 'docs/verification/alarms')
mkdirSync(output, { recursive: true })
const temp = mkdtempSync(resolve(tmpdir(), 'temperature-alarms-'))
...
try {
  ...
  note(`PASS Chrome ${browser.version()}；无非预期控制台异常；API重启保留两条告警。仅验收M2第一小步。`)
} catch(error) {
